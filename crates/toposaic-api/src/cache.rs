...
/// scope here.
const FILE_CATEGORIES: [(&str, &str); 5] = [
    ("elevation", "elevation"),
...
mod tests {
    ...
    async fn clear_all_empties_the_cache_but_never_touches_jobs() {
        ...
        write_file(&cache_dir.join("imagery/s2rgbnir-2021-v200-a.bin"), &[0; 9]);
        ...
        assert_eq!(cleared.removed_bytes, 10 + 20 + 7 + 5 + 9 + 9 + 8);
        assert_eq!(cleared.removed_entries, 7);
        ...
        assert!(!cache_dir.join("imagery").exists());
