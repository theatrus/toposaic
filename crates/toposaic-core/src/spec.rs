...
pub struct GenerationSpec {
    ...
    /// User-placed points that mark a building, paint a dot, cut a flag
    /// socket, or print a label. Empty for setups saved before map markers
    /// existed.
...
impl GenerationSpec {
    ...
    pub fn uses_map_labels(&self) -> bool {
...
impl MarkerKind {
    ...
    pub(crate) fn is_map_label(self) -> bool {
        matches!(self, Self::SurfaceLabel | Self::PlaqueLabel)
    }
}
...
pub struct MapMarker {
    ...
    pub longitude: f64,
    pub kind: MarkerKind,
    #[serde(default = "default_map_label_height_mm")]
...
/// Two layers at the 0.2 mm default layer height: enough that the terrain
/// color, not the cut face, meets the eye at a piece's rim.
fn default_edge_bleed_mm() -> f32 {
    0.4
}
...
impl MapMarker {
    fn validate(&self, base_mm: f32) -> Result<()> {
        ...
        if self.kind.is_map_label() && !(1.5..=12.0).contains(&self.label_height_mm) {
            bail!("map label height must be between 1.5 and 12 mm");
        }
...
pub struct TraySpec {
    ...
    /// Whether the display base carries its embossed place-and-coordinates
    /// label. On by default; a base can be left plain.
...
pub struct ColorOutputSpec {
    ...
    /// How far the terrain color carries down a piece's side wall before the
    /// rock cut face takes over. A wall that is rock to the top draws a grey
    /// outline around every piece as soon as the model is seen from an angle,
    /// worst on hills, where the wall is tallest and faces you. Zero turns the
...
pub struct SlopeGateSpec {
    ...
    /// Reclassify land-cover water as rock where the PRINTED slope exceeds
    /// `water_slope_limit_degrees`. Standing water has no slope: a sea is
    /// level by definition, and a lake surface is flat whatever the hill
    /// around it does. So a water pixel on a steep face is 10 m land cover
    /// bleeding over a shoreline — a seawall, a harbour edge, a cliff —
    /// and printing it climbs the wall in blue.
    ///
    /// Unlike the ground-angle forest and snow gates, both water gates
    /// judge the printed angle: trees answer to real ground, but water on
    /// a wall is a defect of the artifact, and vertical exaggeration is
    /// what builds the wall it climbs.
    ///
    /// Mapped rivers and waterfalls are untouched by either water gate.
    /// Those genuinely do run downhill.
    #[serde(default = "default_water_slope_gate")]
    pub water_slope_gate: bool,
    #[serde(default = "default_water_slope_limit_degrees")]
    pub water_slope_limit_degrees: f32,
    /// The same physics for the water OpenStreetMap maps as polygons — the
    /// sea and lake shapes painted after land cover, which most coastlines
    /// are made of. Separate from the land-cover gate because the sources
    /// err differently: WorldCover bleeds 10 m pixels over shorelines,
    /// while a mapped polygon is usually exact and climbs a wall only
    /// where the DEM and the mapping disagree.
    #[serde(default = "default_water_slope_gate")]
    pub osm_water_slope_gate: bool,
    #[serde(default = "default_water_slope_limit_degrees")]
    pub osm_water_slope_limit_degrees: f32,
}

fn default_water_slope_gate() -> bool {
    true
}

/// Well past any real water surface, and still short of the shallow
/// gradient a coarse DEM shows across a wide estuary, so a genuine
/// shoreline keeps its water.
fn default_water_slope_limit_degrees() -> f32 {
    30.0
}
...
impl Default for SlopeGateSpec {
    fn default() -> Self {
        Self {
            ...
            water_slope_gate: default_water_slope_gate(),
            water_slope_limit_degrees: default_water_slope_limit_degrees(),
            osm_water_slope_gate: default_water_slope_gate(),
            osm_water_slope_limit_degrees: default_water_slope_limit_degrees(),
...
impl SlopeGateSpec {
    fn validate(&self) -> Result<()> {
        ...
        if !(5.0..=85.0).contains(&self.water_slope_limit_degrees) {
            bail!("water slope limit must be between 5 and 85 degrees");
        }
        if !(5.0..=85.0).contains(&self.osm_water_slope_limit_degrees) {
            bail!("mapped water slope limit must be between 5 and 85 degrees");
        }
...
impl ColorOutputSpec {
    fn validate(&self) -> Result<()> {
        ...
        // A practical cap, not a promise the wall is that tall: a mounted
        // back can leave as little as 0.4 mm of side wall, and a bleed deeper
        // than the wall simply takes the whole of it.
...
mod tests {
    ...
    fn full_flat_documents_round_trip_unchanged() {
        let doc: serde_json::Value = serde_json::from_str(
            ...
            "color_output": {
                ...
                "water_slope_gate": false,
                "water_slope_limit_degrees": 25.0,
    ...
    fn map_markers_validate_and_share_the_model_coordinate_frame() {
        ...
        spec.markers[0].kind = MarkerKind::SurfaceLabel;
        spec.markers[0].name = "North Fork".into();
        spec.markers[0].rotation_degrees = 42.0;
        spec.markers[0].label_height_mm = 5.5;
        assert!(spec.uses_map_labels());
        spec.validate().unwrap();
        spec.markers[0].rotation_degrees = 181.0;
        assert!(
            spec.validate()
                .unwrap_err()
                .to_string()
        ...
        spec.markers[0].rotation_degrees = 0.0;
        spec.markers[0].label_height_mm = 1.0;
